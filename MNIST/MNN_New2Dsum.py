...
class _Hitmiss(Function):
    ...
    def forward(self, input, K_hit, K_miss, kernel_size, out_channels):
        ...
        for i in range (out_channels):
            ...
            F_hit = F.max_pool2d(F_hit, kernel_size).view(batch_size, 1, fh, fh, in_channels)
            ...
            F_miss = F.max_pool2d(F_miss, kernel_size).view(batch_size, 1, fh, fh, in_channels)
            
            F_map[:,i] = -1*(F_hit+F_miss).mean(-1)
        
        F_map = F_map.view(batch_size, out_channels, fh, fh)
        return  F_map
