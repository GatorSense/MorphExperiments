...
@author: weihuangxu
"""
# two MNN layers with filter size 6x6, then two fully connection layers. Only test 3,8 in MNIST
...
for epoch in range(10):
    ...
    # use np.where to find the index of 3 and 8 in train set(data and target)

    for batch_idx, (data, target) in enumerate(train_loader):

        loss = 0

        indexTrain38 = np.where(np.logical_or(target.numpy() == 3, target.numpy() == 8))
        # if batch set has no 3 and 8, error saying that numpy array has zero-sized dimensions
        if len(indexTrain38[0]) == 0:
            continue
        x_train = data.numpy()[indexTrain38]
        x_train = Variable(torch.from_numpy(x_train).type(dtype), requires_grad=False)
        y_train = target.numpy()[indexTrain38] - 3
...
for batch_idx, (data, target) in enumerate(test_loader):
    ...
    indexTest38 = np.where(np.logical_or(target.numpy() == 3, target.numpy() == 8))
    # if batch set has no 3 and 8, error saying that numpy array has zero-sized dimensions
    if len(indexTest38[0]) == 0:
        continue
    x_test = data.numpy()[indexTest38]
    x_test = Variable(torch.from_numpy(x_test).type(dtype), requires_grad=False)
    y_test = target.numpy()[indexTest38] - 3
